...
stdenv.mkDerivation {
  ...
  installPhase = ''
    ...
    install -Dm755 packages/coding-agent/dist/omp "$out/bin/omomp"
    install -Dm644 LICENSE "$out/share/doc/omomp/LICENSE"
    install -Dm644 THIRD-PARTY-NOTICES.txt "$out/share/doc/omomp/THIRD-PARTY-NOTICES.txt"
  ...
  preFixup = ''
    remove-references-to -t ${bun} "$out/bin/omomp"
  '';
  ...
  # wrapProgram: the wrapper replaces $out/bin/omomp with a script and moves the
  # ELF to $out/bin/.omomp-wrapped.
  postFixup = lib.optionalString stdenv.hostPlatform.isLinux ''
    patchelf --add-needed libstdc++.so.6 "$out/bin/omomp"
    wrapProgram "$out/bin/omomp" \
  ...
  preInstallCheck = lib.optionalString stdenv.hostPlatform.isLinux ''
    bun ${../scripts/fix-dt-verdef.ts} "$out/bin/.omomp-wrapped"
  '';
  ...
  installCheckPhase = ''
    ...
    smokeOutput="$(HOME="$TMPDIR" "$out/bin/omomp" --smoke-test)"
    grep -q "smoke-test: ok" <<<"$smokeOutput"
    BUN_BE_BUN=1 "$out/bin/omomp" -e \
    ...
    ${lib.optionalString stdenv.hostPlatform.isLinux ''
      ...
      env -u LD_LIBRARY_PATH BUN_BE_BUN=1 "$out/bin/omomp" -e \
      ...
      # wrapProgram moved the real ELF to .omomp-wrapped.
      patchelf --print-needed "$out/bin/.omomp-wrapped" | grep -q '^libstdc++\.so\.6$'
    ''}

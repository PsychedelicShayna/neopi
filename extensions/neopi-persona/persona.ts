...
function setUi(ctx: ExtensionCommandContext, active?: string, warning?: string) {
	ctx.ui.setStatus("neopi-persona", active ? `persona: ${active}` : undefined);
	ctx.ui.setWidget("neopi-persona", warning ? [`Persona warning: ${warning}`] : []);
}
function sessionId(ctx: ExtensionCommandContext) {
...
export function createPersonaFeature(store: BompStateStore, agentRoot: string, api?: ExtensionAPI): PersonaFeature {
	...
	async function activate(name: string, ctx: ExtensionCommandContext): Promise<string> {
		...
		warnings.delete(id);
		annotated.forEach(key => {
			if (key.startsWith(`${id}\0`)) annotated.delete(key);
		});
		api?.appendEntry("neopi_persona", {
			name,
			sessionId: id,
	...
	return {
		...
		async show(name) {
			const d = (await store.read()).personas[name];
			if (!d) throw new Error(`Unknown persona: ${name}`);
			...
			try {
				...
				switch (definition.mode) {
					case "replace":
						...
						break;
					case "prepend":
						systemPrompt = [text, ...event.systemPrompt];
						break;
					case "append":
						systemPrompt = [...event.systemPrompt, text];
						break;
				...
				warnings.delete(id);
				ctx.ui.setStatus("neopi-persona", `persona: ${name}`);
				ctx.ui.setWidget("neopi-persona", []);
				return { systemPrompt };
			} catch (error) {
				const warning = error instanceof Error ? error.message : String(error);
				warnings.set(id, warning);
				ctx.ui.setStatus("neopi-persona", `persona: ${name} (warning)`);
				ctx.ui.setWidget("neopi-persona", [`Persona warning: ${warning}`]);
				const key = `${id}\0${name}\0${warning}`;
				if (!annotated.has(key)) {
					annotated.add(key);
					api?.appendEntry("neopi_persona_warning", { command: `/persona use ${name}`, name, warning });
				}
				return;
			}
		},

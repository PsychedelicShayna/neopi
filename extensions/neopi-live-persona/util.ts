import type { ExtensionCommandContext } from "../../packages/coding-agent/src/extensibility/extensions/types.ts";

export function words(args: string): string[] {
	return args.trim().split(/\s+/).filter(Boolean);
}
export function output(ctx: ExtensionCommandContext, text: string, error = false): void {

// neopi-loadout: atomic runtime model loadout switching.
// Self-contained extension directory — drag into any omp extensions dir to
// enable, drag out to disable. State lives in <agentDir>/neopi-loadout.json.
...
async function loadoutMenu(loadouts: LoadoutFeature, ctx: ExtensionCommandContext): Promise<void> {
	...
	while (true) {
		...
		switch (action) {
			case "Use":
				ctx.ui.notify(await loadouts.use(loadoutName, ctx), "info");
				break;
			case "Show":
				ctx.ui.notify(await loadouts.show(loadoutName), "info");
				break;
			case "Deactivate":
				ctx.ui.notify(await loadouts.off(ctx), "info");
				break;
...
export default function neopi_loadout(api: ExtensionAPI): void {
	...
	api.registerCommand("loadout", {
		...
		async handler(args: string, ctx: ExtensionCommandContext) {
			...
			switch (command) {
				case "list":
					return report(ctx, () => loadouts.list());
				case "show":
					if (!name) return output(ctx, "Usage: /loadout show <name>", true);
					return report(ctx, () => loadouts.show(name));
				case "status":
					return report(ctx, () => loadouts.status());
				case "use":
					if (!name) return output(ctx, "Usage: /loadout use <name>", true);
					return report(ctx, () => loadouts.use(name, ctx));
				case "off":
					return report(ctx, () => loadouts.off(ctx));
				default:
					return output(ctx, "Usage: /loadout list|show <name>|use <name>|off|status", true);
			}
		},
	});
}

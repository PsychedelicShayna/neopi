...
function setUi(ctx: ExtensionCommandContext, name?: string) {
	ctx.ui.setStatus("neopi-loadout", name ? `loadout: ${name}` : undefined);
	ctx.ui.setWidget("neopi-loadout", name ? [`Active model loadout: ${name}`] : []);
}
export function createLoadoutFeature(store: BompStateStore): LoadoutFeature {
	...
	function requireLoadoutApi(ctx: ExtensionCommandContext): void {
		if (typeof ctx.applyRuntimeModelLoadout !== "function")
			throw new Error(
				"This NeoPi build lacks runtime model loadouts (ctx.applyRuntimeModelLoadout); /loadout use|off needs the patched build",
			);
	}
	return {
		data,
		...
		async off(ctx) {
			...
			try {
				...
				if (previous && state.loadouts[previous]) await ctx.applyRuntimeModelLoadout(state.loadouts[previous]);
				throw error;
			}
			setUi(ctx);
			return "Loadout off.";

...
} from "../../packages/coding-agent/src/eval/backend.ts";
...
export interface ShellProfileRecord {
	alias: string;
	shell?: "sh" | "fish" | "zsh" | "xonsh" | string;
	executable?: string;
...
function statusMarker(profile: ShellProfileRecord, executable: string, marker: string): string {
	switch (shellDialect(profile, executable)) {
		...
		case "xonsh":
			return `print('${marker}' + str($LAST_RETURN_CODE))`;
		default:
			return `printf '${marker}%s\\n' $?`;
	}
}
...
class PersistentShell {
	...
	constructor(
		readonly profile: ShellProfileRecord,
	...
	execute(code: string, options: ExecutorBackendExecOptions): Promise<ExecutorBackendResult> {
		const pending = this.#queue.then(() => this.#execute(code, options));
	...
	async #executeFish(code: string, options: ExecutorBackendExecOptions): Promise<ExecutorBackendResult> {
		...
		const script = `${code}\nset -l __neopi_status $status\nprintf '\\\\0%s\\\\0%d\\\\0%s\\\\0' ${shellQuote(marker)} $__neopi_status (pwd)\nenv -0`;
		const command = [
			"stty -echo; exec",
			shellQuote(this.executable),
			...(this.profile.args ?? []).map(shellQuote),
			"-c",
			shellQuote(script),
		].join(" ");
		...
		try {
			...
			for (const field of fields) {
				...
				if (equals > 0) env[field.slice(0, equals)] = field.slice(equals + 1);
			}
			this.#fishEnv = env;
	...
	#start(cwd: string): void {
		...
		const child = Bun.spawn(["/usr/bin/script", "-qefc", command, "/dev/null"], {
			...
			env: {
				...process.env,
				...this.profile.env,
				TERM: dialect === "fish" ? "dumb" : (process.env.TERM ?? "xterm-256color"),
			},
			stdin: "pipe",
			stdout: "pipe",
			stderr: "pipe",
	...
	async #waitForChunk(timeoutMs: number): Promise<void> {
		...
		const timer = setTimeout(
			() => deferred.reject(new Error(`Shell ${this.profile.alias} cell timed out`)),
			timeoutMs,
		);
		timer.unref?.();
		try {
	...
	async #stop(): Promise<void> {
		...
		for (const active of [child, fishChild]) {
			...
			try {
				process.kill(process.platform === "win32" ? active.pid : -active.pid, "SIGTERM");
			} catch {}
			await Promise.race([active.exited, Bun.sleep(500)]).catch(() => undefined);
			if (active.exitCode === null) {
				try {
					process.kill(process.platform === "win32" ? active.pid : -active.pid, "SIGKILL");
				} catch {}
			}
		}
...
export async function discoverShellBackends(profiles: readonly ShellProfileRecord[]): Promise<ExtensionEvalBackend[]> {
	const records: ShellProfileRecord[] = [{ alias: "sh", shell: "sh" }, ...profiles];
	...
	for (const profile of records) {
		...
		if (!TOKEN.test(alias) || claimed.has(alias)) continue;
		const executable = await resolveExecutable(profile);
		if (!executable) continue;
		claimed.add(alias);
		const runtime = new PersistentShell(profile, executable);
		backends.push({
			id: alias,
			aliases: [],
			label: alias === "sh" ? "Shell" : `Shell (${alias})`,
			highlightLang: profile.shell === "xonsh" ? "python" : "shell",
			modelVisible: true,
			isAvailable: () => runtime.available,
			execute: (code, options) => runtime.execute(code, options),
			reset: () => runtime.reset(),

import type { ExecutorBackendResult } from "../../packages/coding-agent/src/eval/backend.ts";
import type { EvalDisplayOutput } from "../../packages/coding-agent/src/eval/types.ts";
import type { ExtensionEvalBackend } from "../../packages/coding-agent/src/extensibility/extensions/types.ts";

export interface KernelAliasRecord {
	alias: string;
	kernel: string;
	env?: Readonly<Record<string, string>>;
}
export interface JupyterRuntimeSnapshot {
	alias: string;
	kernelName: string;
	displayName: string;
...
interface KernelSpec {
	resource_dir: string;
	spec: { argv: string[]; display_name: string; language: string; env?: Record<string, string> };
}
interface Pending {
...
async function renderKernelDisplay(
	content: Record<string, unknown>,
): Promise<{ text: string; outputs: EvalDisplayOutput[] }> {
...
export function getJupyterRuntimeSnapshot(): readonly JupyterRuntimeSnapshot[] {
...
export async function interruptJupyterKernel(alias: string): Promise<boolean> {
	...
	await activeBridge.request("interrupt", { alias });
	return true;
}
export async function restartJupyterKernel(alias: string): Promise<boolean> {
	const state = states.get(alias);
	if (!state || !activeBridge) return false;
	state.state = "starting";
	try {
		await activeBridge.request("restart", { alias });
		state.state = "idle";
		return true;
	} catch {
		state.state = "error";
...
async function discoverKernelSpecs(): Promise<{ python: string; specs: Record<string, KernelSpec> } | undefined> {
	const candidates = [
		...new Set(
			["/usr/bin/python3", "/usr/bin/python", Bun.which("python3"), Bun.which("python")].filter(
	...
	for (const python of candidates) {
		...
		const child = Bun.spawn([python, "-m", "jupyter", "kernelspec", "list", "--json"], {
			stdin: "ignore",
			stdout: "pipe",
			stderr: "pipe",
		});
...
class JupyterBridge {
	...
	#start(): void {
		...
		this.#process = Bun.spawn([this.#python, `${import.meta.dir}/jupyter-sidecar.py`], {
			stdin: "pipe",
			stdout: "pipe",
			stderr: "pipe",
	...
	async #read(child: Bun.Subprocess<"pipe", "pipe", "pipe">): Promise<void> {
		...
		try {
			while (true) {
				...
				while (true) {
					...
					if (message.event === "chunk") {
						pending.onChunk?.(String(message.text ?? ""));
						continue;
					}
					if (message.event === "display") continue;
					if (message.event === "stdin") continue;
					this.#pending.delete(id);
					if (message.ok === false) pending.reject(new Error(String(message.error ?? "Jupyter request failed")));
			...
			this.#failAll(new Error("Jupyter sidecar closed"));
		}
	}
	...
	request(
		...
		if (signal?.aborted)
			return Promise.reject(signal.reason instanceof Error ? signal.reason : new Error("Jupyter request aborted"));
		this.#start();
		const id = `j${++this.#sequence}`;
		const deferred = Promise.withResolvers<Record<string, unknown>>();
		const pending: Pending = { resolve: deferred.resolve, reject: deferred.reject, onChunk };
	...
	async dispose(): Promise<void> {
		...
		this.#failAll(new Error("Jupyter sidecar disposed"));
...
export async function discoverJupyterBackends(
	configured: readonly KernelAliasRecord[],
): Promise<ExtensionEvalBackend[]> {
	...
	const bridge = new JupyterBridge(python);
	activeBridge = bridge;
	states.clear();
	const claimed = new Set<string>();
	const selected: KernelAliasRecord[] = [];
	const coconut = Object.keys(specs).find(name => name.toLowerCase() === "coconut");
	if (coconut) selected.push({ alias: "coconut", kernel: coconut });
	for (const record of configured) selected.push(record);
	const backends: ExtensionEvalBackend[] = [];
	for (const record of selected) {
		const alias = record.alias.trim();
		const spec = specs[record.kernel];
		if (!TOKEN.test(alias) || claimed.has(alias) || !spec) continue;
		claimed.add(alias);
		...
		backends.push({
			...
			highlightLang: spec.spec.language || "text",
			modelVisible: true,
			isAvailable: () => states.has(alias),
			async execute(code, options) {
				...
				try {
					const response = await bridge.request(
						"execute",
						{ alias, kernel_name: record.kernel, cwd: options.cwd, env: record.env ?? {}, code },
						chunk => {
							streamed += chunk;
					...
					const bundles = Array.isArray(response.bundles) ? (response.bundles as Record<string, unknown>[]) : [];
					const displays: EvalDisplayOutput[] = [];
					let rendered = "";
					for (const bundle of bundles) {
						const display = await renderKernelDisplay(bundle);
						rendered += display.text;
						displays.push(...display.outputs);
					}
					if (rendered) options.onChunk(rendered);
					state.state = "idle";
					return result(streamed + rendered, response.ok === false ? 1 : 0, displays);
				} catch (error) {
					state.state = options.signal?.aborted ? "idle" : "error";
					if (options.signal?.aborted) return { ...result(streamed, undefined, []), cancelled: true };
					throw error;
				}
			},
			reset: async () => {
				await restartJupyterKernel(alias);
			},
			interrupt: async () => {
				await interruptJupyterKernel(alias);
			},
			dispose: async () => {

...
import {
	discoverJupyterBackends,
	getJupyterRuntimeSnapshot,
	interruptJupyterKernel,
	restartJupyterKernel,
	type KernelAliasRecord,
} from "./jupyter";
...
export interface ReplProfileConfiguration {
	shellProfiles: readonly ShellProfileRecord[];
	kernelAliases: readonly KernelAliasRecord[];
}
export interface ReplBackendItem {
	id: string;
	label: string;
	kind: "agent" | "builtin" | "shell" | "kernel";
}
export interface ReplRuntimeSnapshot {
	mode: "agent" | "eval";
	active: string;
	available: readonly ReplBackendItem[];
}
...
const DEFAULT_BACKENDS: readonly ReplBackendItem[] = [
...
export function getReplRuntimeSnapshot(): ReplRuntimeSnapshot {
	return { mode: active === "agent" ? "agent" : "eval", active, available: [...available] };
}
...
export function setReplBackend(id: string): boolean {
	if (!available.some(item => item.id === id)) return false;
	active = id;
	return true;
}
export function cycleReplBackend(): string {
	const index = available.findIndex(item => item.id === active);
	active = available[(index + 1) % available.length]?.id ?? "agent";
	return active;
}
...
async function replMenu(ctx: ExtensionContext): Promise<void> {
	...
	if (setReplBackend(backend.id)) {
		ctx.ui.setStatus("neopi-repl", backend.id === "agent" ? undefined : `REPL ${backend.id}`);
...
async function kernelMenu(ctx: ExtensionContext): Promise<void> {
	...
	if (snapshots.length === 0) {
		ctx.ui.notify('No kernels — map an alias under "kernelAliases" in neopi-repl.json', "info");
		return;
	}
	...
	while (true) {
		...
		switch (action) {
			case "Interrupt": {
				...
				ctx.ui.notify(
					ok ? `Interrupted ${kernel.alias}` : `Failed to interrupt ${kernel.alias}`,
					ok ? "info" : "error",
				);
				break;
			}
			case "Restart": {
				if (await ctx.ui.confirm("Restart kernel", `Restart '${kernel.alias}'? Running cells will be lost.`)) {
					...
					ctx.ui.notify(
						ok ? `Restarted ${kernel.alias}` : `Failed to restart ${kernel.alias}`,
						ok ? "info" : "error",
					);
				}
				break;
...
export const createReplExtension: ExtensionFactory = api => {
	...
	api.on("session_start", async (_event, ctx) => {
		if (!registered) {
			...
			if (typeof api.registerEvalBackend !== "function") {
				ctx.ui.notify(
					"neopi: this build lacks registerEvalBackend — shell/kernel REPL backends disabled (builtins still work)",
					"warning",
				);
			} else {
				void (async () => {
					try {
						...
						for (const [kind, backends] of [
							["shell", shells],
							["kernel", kernels],
						] as const) {
							for (const backend of backends) {
								if (claimed.has(backend.id)) {
						...
						registered = false;
						ctx.ui.notify(
							`neopi: REPL backend discovery failed: ${error instanceof Error ? error.message : String(error)}`,
							"error",
						);
					}
				})();
		...
		unsubscribeInput = ctx.ui.onTerminalInput(data => {
			if (active !== "agent" && data === "\x1b") {
				active = "agent";
				ctx.ui.setStatus("neopi-repl", undefined);
				ctx.ui.notify("Returned to agent", "info");
			}
			return undefined;
		});
	});
		...
		async handler(args, ctx): Promise<void> {
			...
			if (!requested) {
				if (ctx.hasUI) await replMenu(ctx);
					...
					ctx.ui.notify(
						getReplRuntimeSnapshot()
							.available.map(b => `${b.id === active ? "*" : "-"} ${b.label} (${b.kind})`)
							.join("\n"),
						"info",
					);
				return;
			}
			if (!setReplBackend(requested)) {
				ctx.ui.notify(`Unavailable REPL backend: ${requested}`, "error");
				return;
			}
			ctx.ui.setStatus("neopi-repl", active === "agent" ? undefined : `REPL ${active}`);
	...
	api.registerCommand("kernel", {
		...
		async handler(args, ctx): Promise<void> {
			...
			if (ctx.hasUI) await kernelMenu(ctx);
				...
				ctx.ui.notify(
					getJupyterRuntimeSnapshot()
						.map(k => `${k.alias}: ${k.displayName} (${k.state})`)
						.join("\n") || "No kernels",
					"info",
				);
		},
	...
	api.registerShortcut("alt+r", {
		...
		handler(ctx): void {
			...
			ctx.ui.setStatus("neopi-repl", selected === "agent" ? undefined : `REPL ${selected}`);

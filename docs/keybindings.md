...
| `app.stt.toggle`             | `ctrl+space`                                                         | Record a complete utterance for native xAI transcription. Press once to record and again to stop; pauses never split the recording. Uses existing xAI credentials independently of configured dictation. |
| `app.dictation.toggle`       | `ctrl+alt+space`                                                      | Toggle the configured Dictation model. The separate Space-hold gesture also uses this pipeline and requires `stt.enabled`. Neither route stops a Ctrl+Space recording. |

...
| `app.stt.toggle`             | Unbound (hold `Space`)                                                | Toggle speech-to-text. By default there is no key chord — hold the space bar to record (push-to-talk) and release to transcribe; bind a chord here for a press-to-toggle alternative. The two routes never fight: while a capture is in flight the space-hold gesture is inert, and a release only stops the recording that same hold started, so a chord-started dictation survives an accidental space-bar hold |

...
mkdir -p "$BINARY_DIR"
cp packages/coding-agent/dist/npi "$BINARY_DIR/npi"
smoke_cli "$BINARY_DIR/npi"
...
#    `bin.npi` from `src/cli.ts` to the prepack bundle `dist/cli.js`. The repo
...
(
   ...
   smoke_cli ./node_modules/.bin/npi
)

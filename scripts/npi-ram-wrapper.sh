...
readonly REAL_NPI="${NPI_REAL_BINARY:-/usr/local/libexec/npi-portable}"
readonly PORTABLE_BUNDLE="${NPI_PORTABLE_BUNDLE:-/usr/local/lib/npi-portable}"
readonly FILTER_FILE="${NPI_RAM_FILTER:-$PORTABLE_BUNDLE/ram-filter.rules}"
readonly SYNC_POLICY="${NPI_RAM_SYNC:-ask}"
export NPI_PORTABLE_BUNDLE="$PORTABLE_BUNDLE"

if [[ ! -x "$REAL_NPI" ]]; then
	printf 'npi: portable executable is missing or not executable: %s\n' "$REAL_NPI" >&2
	exit 127
fi
if [[ ! -r "$FILTER_FILE" ]]; then
	printf 'npi: RAM filter is missing or unreadable: %s\n' "$FILTER_FILE" >&2
	exit 78
fi
case "$SYNC_POLICY" in
	ask | always | never) ;;
	*)
		printf 'npi: NPI_RAM_SYNC must be ask, always, or never (got %s)\n' "$SYNC_POLICY" >&2
		exit 2
		;;
esac

if [[ "${NPI_RAM_DISABLE:-0}" == "1" ]]; then
	exec "$REAL_NPI" "$@"
fi
...
if ((interactive)) && [[ -z "${TMUX:-}" && -z "${NPI_TMUX_BOOTSTRAPPED:-}" ]]; then
	exec 8>"$LOCK_FILE"
	if ! flock -n 8; then
		printf 'npi: another npi is using this RAM profile; exit it first, or run NPI_RAM_DISABLE=1 npi\n' >&2
		exit 75
	fi
	flock -u 8
	exec 8>&-
	...
	export -p >"$env_file"
	set +e
	tmux new-session -s "npi-${UID}-$$" -- env \
		NPI_TMUX_BOOTSTRAPPED=1 \
		NPI_TMUX_ENV_FILE="$env_file" \
		"$0" "$@"
	tmux_status=$?
	set -e
	rm -f -- "$env_file"
...
if [[ ! -e "$INIT_MARKER" || "${NPI_RAM_REFRESH:-0}" == "1" ]]; then
...
binary_bytes="$(stat -c '%s' -- "$REAL_NPI")"
...
cap="${NPI_RAM_MAX_BYTES:-$default_cap}"
if [[ ! "$cap" =~ ^[0-9]+$ ]]; then
	printf 'npi: NPI_RAM_MAX_BYTES must be an integer byte count (got %s)\n' "$cap" >&2
	exit 2
fi
if ((required_bytes > cap)); then
	printf 'npi: RAM profile requires %s bytes but cap is %s bytes (shm available=%s, MemAvailable=%s, default cap=%s)\n' \
		"$required_bytes" "$cap" "$shm_available" "$mem_available" "$default_cap" >&2
	printf 'npi: run NPI_RAM_DISABLE=1 npi to use persistent storage directly\n' >&2
	exit 75
fi
fi

if [[ ! -e "$INIT_MARKER" || "${NPI_RAM_REFRESH:-0}" == "1" ]]; then
	printf 'npi: loading bounded agent state into RAM from %s (%s bytes)\n' "$persistent_agent_dir" "$required_bytes" >&2
	...
	install -m 0700 -- "$REAL_NPI" "$stage/bin/npi"
	rm -f -- "$INIT_MARKER"
...
case "$SYNC_POLICY" in
	...
	ask)
		if ((interactive)) && [[ -r /dev/tty && -w /dev/tty ]]; then
			...
			printf 'npi: RAM state remains unsynchronized at %s\n' "$RAM_AGENT" >&2
		fi
		;;
esac
...
if ((sync_requested)); then
	printf 'npi: synchronizing bounded RAM state to %s\n' "$persistent_agent_dir" >&2
	...
	if ((sync_failed)); then
		printf 'npi: synchronization failed; RAM state retained at %s\n' "$RAM_AGENT" >&2
	else
		date -u +'%Y-%m-%dT%H:%M:%SZ' >"$RAM_PROFILE/.last-sync"
		printf 'npi: synchronization complete\n' >&2
	fi
else
	printf 'npi: RAM state retained at %s (until sync, discard, or reboot)\n' "$RAM_AGENT" >&2
fi

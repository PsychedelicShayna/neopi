...
export class AgentSession {
	...
	constructor(config: AgentSessionConfig) {
		...
		const evalKernelOwnerId = config.evalKernelOwnerId ?? `agent-session:${Snowflake.next()}`;
		const evalToolSession: ToolSession = config.toolSession ??
			config.evalToolSession ?? {
				get cwd() {
		...
		this.#eval = new EvalRunner(evalHost, {
			kernelOwnerId: evalKernelOwnerId,
	...
	sendCustomMessageWithReceipt<T = unknown>(
		...
		const completion = this.#sendCustomMessageWithAcceptance(
			message,
			options,
			accept,
			() => cancelled,
	...
	async #sendCustomMessageWithAcceptance<T = unknown>(
		...
		return this.#admitSubmission(async () => {
			...
			try {
				...
				return await this.#dispatchCustomMessage(
					message,
					options,
					outcome,
					onAccepted,
	...
	async #dispatchCustomMessage<T = unknown>(
		...
		if (options?.deliverAs === "aside") {
			if (await this.#sessionGenerationChanged(sessionGeneration)) return false;
			if (this.#planModeState?.enabled || this.#advisors.autoResumeSuppressed) {

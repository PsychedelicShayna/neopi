...
/** Row shape read from an `npi stats` messages table during backfill. */
...
`);
	 ...
	 * `npi stats` database (`~/.omp/stats.db`) into model_perf. Fire-and-forget:
	 ...
	 * A missing stats.db leaves the marker unset so a later `npi stats` run
	...
	/**
	 * Imports recent measurable request rows from an `npi stats` database

 ...
 * URLs) are always written directly to disk — those are NeoPi-owned and should
 ...
 * active plan file while plan mode is enabled — both are NeoPi-internal artifacts
...
export async function shouldRouteWriteThroughBridge(
	...
	// NeoPi-owned session artifacts (plan files, scratch notes) must stay off the

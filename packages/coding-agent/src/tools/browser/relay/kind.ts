 ...
 * relay served by `npi browser-relay` (sibling `server.ts`/`bridge.ts`) plus
 * its companion extension (`packages/browser-relay`, installed via
 * `npi browser-relay install`). The relay impersonates Chrome's CDP discovery
...
/** Browser kind selecting the NeoPi browser relay. */
export interface RelayKind {
	kind: "relay";
	cdpUrl: string;
}

/** Default endpoint of the `npi browser-relay` CLI. */

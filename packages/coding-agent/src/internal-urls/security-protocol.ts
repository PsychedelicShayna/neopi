...
import { PRODUCT_NAME, sanitizeText } from "@oh-my-pi/pi-utils";
		...
		if (parts.length === 0) {
			return createSecurityResource({
				...
				content: [
					"# Security",
					"",
					`${PRODUCT_NAME}-owned software-security analysis resources. The namespace is read-only; use explicit security commands or tools for mutations.`,
					"",

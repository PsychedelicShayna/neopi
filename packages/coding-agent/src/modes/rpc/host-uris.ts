import { PRODUCT_NAME, Snowflake } from "@oh-my-pi/pi-utils";
...
export class RpcHostUriBridge {
	...
	setSchemes(schemes: RpcHostUriSchemeDefinition[]): string[] {
		...
		for (const raw of schemes) {
			...
			// Built-in schemes are harness-owned: a host shadowing one would change its semantics for
			// the whole process, and `clear()` would then delete it for later sessions.
			if (this.#router.isBuiltin(scheme)) {
				throw new Error(`Host URI scheme is reserved by ${PRODUCT_NAME}: ${scheme}://`);
			}

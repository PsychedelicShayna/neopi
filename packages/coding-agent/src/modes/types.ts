...
export interface InteractiveModeContext {
	...
	handleDictationToggle(): Promise<void>;
	/** Space-bar push-to-talk into `target`: a recognized hold starts dictation and its release stops
	 *  it. Gated on `stt.enabled` and {@link sttIdle}, so a disabled STT leaves the space bar typing
	 *  normally and a hold never engages on top of a chord-started capture. */

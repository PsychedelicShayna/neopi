 ...
 * doesn't look like another flag. Without this, `npi --system-prompt --profile
 ...
 * `npi --some-ext-flag --profile work` still selects a profile. Known value-less
 * launch flags ({@link VALUELESS_FLAGS}) are exempt so a trailing profile after
 * them also activates (`npi --print --profile work`).
 */
 ...
 * positionals are never stolen (`npi grep --profile <path>` greps for
 * `--profile`; it does not select a profile). `launch` and `acp` are explicit
 * spellings of launch-shaped commands, so `npi launch --profile work` and
 * `npi acp --profile work` still select profile `work`.
 *
...
export function extractProfileFlags(argv: readonly string[]): ProfileBootstrapResult {
	...
	for (let index = 0; index < argv.length; index += 1) {
		...
		// successor only when it is value-like — `npi --bar val --profile work` keeps
		...
		// a trailing global flag still applies (`npi --some-ext-bool --profile work`
		...
		// activates (`npi --print --profile work`).

 ...
 *
 * Handles `npi usage` — fetches provider usage reports for every
...
/**
 * Tombstones worth a row in `npi usage`: OAuth credentials torn down
 ...
 * `npi usage --provider xai` can still confirm the stored credential has no
 * usage endpoint.
 */

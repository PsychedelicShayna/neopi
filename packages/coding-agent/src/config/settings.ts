...
export class Settings {
	...
	/**
	 * Atomically replace or clear the volatile model loadout maps (model roles, retry fallback
	 * chains, task agent model overrides). Validation and cloning complete before the override
	 * layer is mutated; listeners fire once, after all three maps are in place.
	 */
	applyRuntimeOverridesAtomically(overrides: RuntimeModelOverrides | undefined): void {
		...
		const assignOrDelete = (segments: string[], value: unknown): void => {
			if (value !== undefined) setByPath(nextOverrides, segments, value);
			else deleteByPath(nextOverrides, segments);
		};
		const source = validated ?? this.#runtimeModelOverrideBaseline;
		assignOrDelete(["modelRoles"], source?.modelRoles);
		assignOrDelete(["retry", "fallbackChains"], source?.retryFallbackChains);
		assignOrDelete(["task", "agentModelOverrides"], source?.taskAgentModelOverrides);

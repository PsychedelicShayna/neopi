...
7. Run focused regression coverage and required repository checks with `CARGO_BUILD_JOBS=6`. Fix update-caused failures. Record exact commands, results, and any independently established pre-existing failures; NEVER claim unrun checks passed.
8. Build with `./build.sh` from the sync worktree; NEVER assemble the build from individual `bun run` commands. It rebuilds the native addon when stale and never deploys extensions. Verify the version, help, worker smoke probe, and affected runtime paths. Stage with `NPI_DEST=<temp>/bin/npi PI_CODING_AGENT_DIR=<temp>/agent ./install.sh` and exercise that executable before merging the PR.

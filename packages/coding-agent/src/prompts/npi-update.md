...
7. Run focused regression coverage, required repository checks, and the native build with `CARGO_BUILD_JOBS=6`. Fix update-caused failures. Record exact commands, results, and any independently established pre-existing failures; NEVER claim unrun checks passed.
8. Build `packages/coding-agent/dist/npi` with automatic extension deployment disabled (`NPI_SKIP_EXTENSION_INSTALL=1`). Verify the version, help, worker smoke probe, and affected runtime paths. Install to a staging location and exercise that executable before merging the PR.
...
11. Back up the existing `npi` executable, then atomically install only `npi` at its existing path (normally `~/.local/bin/npi`). Deploy extensions from the stable checkout with `bun scripts/install-neopi-extensions.ts`. Preserve unrelated extensions and backups.

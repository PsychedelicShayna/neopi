 ...
 * SSH JSON Provider
 *
 * Discovers SSH hosts from managed npi config paths and legacy root ssh.json files.
...
registerProvider(sshCapability.id, {
	...
	description: "Load SSH hosts from managed npi paths and legacy ssh.json/.ssh.json files",
	priority: 5,
	load,
});

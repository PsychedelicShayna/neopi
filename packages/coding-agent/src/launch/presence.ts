 ...
 * deleting a scope whose owning NeoPi process is mid-startup (token written, broker
...
/** Handle keeping one NeoPi process registered in a project daemon scope. */
export interface DaemonProjectPresence {
	close(): Promise<void>;
}

/** Register this NeoPi process so project daemons survive while it remains alive. */
...
/** Return whether a registered NeoPi process in this runtime directory is still alive. */

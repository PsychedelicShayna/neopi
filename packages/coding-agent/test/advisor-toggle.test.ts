...
describe("AgentSession advisor toggle", () => {
	...
	it("replaces only the advisor base prompt and rebuilds on override changes and reset", async () => {
		...
		try {
			...
			const defaultPrompt = [...defaultAdvisor.state.systemPrompt];
			const appended = [
				"Project context",
				"Memory instructions",
			...
			expect(defaultPrompt.slice(1)).toEqual(appended);
			...
			promptSession.setAdvisorEnabled(true);
			expect(promptSession.getAdvisorAgent()?.state.systemPrompt).toEqual(defaultPrompt);

...
describe("AgentSession auto-compaction queue resume", () => {
	...
	it.each(["plain", "receipt", "receipt-after-cancellation"] as const)(
		"lets a %s custom-message turn from a parked command replace the resume",
		async delivery => {
			...
			type Dispatched = { role: string; customType?: string; synthetic?: boolean };
			const prompted: Dispatched[][] = [];
			vi.spyOn(session.agent, "prompt").mockImplementation(async (message, options) => {
				if (!Array.isArray(options)) options?.onAccepted?.();
				prompted.push((Array.isArray(message) ? message : [message]) as Dispatched[]);
			});
			...
			(globalThis as ParkCommandGlobals).__ompParkAction = () => {
				const message = {
					customType: "extension-directive",
					content: "carry on with the new plan",
					display: false,
				};
				if (delivery !== "plain") {
					if (delivery === "receipt-after-cancellation") {
						const predecessor = session.sendCustomMessageWithReceipt(
							{ ...message, content: "cancelled directive" },
							{ triggerTurn: true },
						);
						void predecessor.accepted.catch(() => {});
						void predecessor.completed.catch(() => {});
						expect(predecessor.cancel()).toBe(true);
					}

					const receipt = session.sendCustomMessageWithReceipt(message, { triggerTurn: true });
					sent = Promise.all([receipt.accepted, receipt.completed]).then(([, completed]) => completed);
				} else {
					sent = session.sendCustomMessage(message, { triggerTurn: true });
				}
			};
			const compactGate = Promise.withResolvers<void>();
			(globalThis as typeof globalThis & { __ompManualCompactGate?: Promise<void> }).__ompManualCompactGate =

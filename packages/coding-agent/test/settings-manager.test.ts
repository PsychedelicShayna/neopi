...
describe("Settings", () => {
	...
	it("persists capture independently of Local memory and excludes all subagents", async () => {
		...
		cfgMemoryBackend.set(settings, "local");
		cfgChroniclerEnabled.set(settings, true);
		await settings.flush();
		resetSettingsForTest();
		const reloaded = await Settings.init({ cwd: projectDir, agentDir });
		expect(cfgMemoryBackend.get(reloaded)).toBe("local");
		expect(cfgChroniclerEnabled.get(reloaded)).toBe(true);
		expect(cfgChroniclerEnabled.get(createSubagentSettings(reloaded, { "chronicler.enabled": true }))).toBe(false);
		expect(cfgChroniclerEnabled.get(reloaded)).toBe(true);
	});

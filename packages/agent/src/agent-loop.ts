...
async function executeToolCalls(
	...
	const checkSteering = async (): Promise<void> => {
		...
		let steeringInterruptMode = interruptMode;
	...
	// promptly. A queued message may carry its own interrupt mode, so steering is
	// watched whenever the queue is observable; asides follow the loop mode, where
	// "wait" only needs the watch for a batch holding an interruptible wait.
	...
	const watchSteeringWhileRunning =
		hasSteeringMessages !== undefined ||
		((softInterrupts || records.some(record => record.interruptible)) && hasAsidePeek);

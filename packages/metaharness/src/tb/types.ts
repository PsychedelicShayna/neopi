 ...
 * npi runs inside that guest over raw RPC stdio carried by the Vibemon SDK's
 ...
 * - `agent.ts` — npi Linux binary build + guest install
...
/** Guest CPU architecture, normalized to npi binary naming. */
...
/** Per-run NeoPi configuration installed into every guest. */
export interface AgentConfig {
	/** Tool allowlist passed to `npi` as `--tools`. */
	tools: string[];
	/** Extra environment for the `npi` process only; never reaches the verifier. */
...
/** OpenRouter request-routing suffix applied by NeoPi. */
...
export interface GatewayConfig {
	...
	url: string;
	/** Gateway bearer (`npi auth-gateway token`); `"no-auth"` when the gateway runs open. */
	token: string;
...
/** Host paths of self-contained npi Linux binaries, keyed by guest architecture. */
export interface AgentBinaries {
	x64?: string;
	arm64?: string;
	/** NeoPi package version the binaries were built from (for run provenance). */
	version: string;
}

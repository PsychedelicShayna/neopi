...
/** Build and cache self-contained npi binaries for the selected guest architectures. */
export async function prepareAgentBinaries(opts: {
	...
	for (const arch of arches) {
		cached[arch] = path.join(opts.cacheDir, `npi-linux-${arch}-${manifest.version}`);
	}
	...
	if (missing.length > 0) {
		...
		for (const arch of missing) {
			const source = path.join(CODING_AGENT_DIR, "binaries", `npi-linux-${arch}`);
...
/** Install npi and gateway-only configuration into one running trial microVM. */
export async function installAgent(
	...
	if (!binary) throw new Error(`No npi binary available for guest architecture ${vm.arch}`);

	const entrypoint = "/opt/omp/npi";
	...
	if (chmod.exitCode !== 0) throw new Error(`Could not make npi executable: ${chmod.stderr.trim()}`);
	const warmup = await vm.exec(`${shellQuote(entrypoint)} --version`);
	if (warmup.exitCode !== 0) throw new Error(`npi --version failed: ${warmup.stderr.trim()}`);
...
`;
	...
	if (writeConfig.exitCode !== 0)
		throw new Error(`Could not install NeoPi configuration: ${writeConfig.stderr.trim()}`);
	return entrypoint;
}

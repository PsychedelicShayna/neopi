...
import { PRODUCT_NAME, setTerminalHeadless } from "@oh-my-pi/pi-utils";
...
describe("terminal notifications", () => {
	...
	it("formats structured Kitty OSC 99 title and body chunks", () => {
		...
		expect(out).toBe(
			`\x1b]99;i=complete-1:f=${Buffer.from(PRODUCT_NAME).toString("base64")}:a=focus:u=1:t=Y29tcGxldGlvbg==:n=aW5mbw==:s=aW5mbw==:w=5000:d=0;Session\x1b\\` +
	...
	it("base64-encodes unsafe OSC 99 payload controls", () => {
		...
		expect(out).toBe(
			`\x1b]99;i=unsafe:f=${Buffer.from(PRODUCT_NAME).toString("base64")}:e=1;TGluZSAxCkxpbmUgMg==\x1b\\`,
		);
	});
	...
	it("never hands Herdr a title it would read as a help request", () => {
		...
		expect(["help", "--help", "-h"]).not.toContain(titles[0]);
		expect(titles[1]).toBe("-x session");
	});

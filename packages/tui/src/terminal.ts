...
export function emergencyTerminalRestore(): void {
	try {
		...
		if (terminal) {
			...
			// frame before leaving it, then let stop() pop NeoPi's main-screen frame.
			if (altScreenActive) {
				const keyboardExit =
...
export class ProcessTerminal implements Terminal {
	...
	#attachInput(): void {
		...
		// `rmkx` = "\x1b[?1l\x1b>"). NeoPi decodes both CSI ("\x1b[A") and SS3
		...
		// are disabled while NeoPi owns the TTY so typing in the editor does not
	...
	stop(): void {
		...
		// navigation stays broken after NeoPi exits (#6374).
		this.#safeWrite("\x1b[?1l\x1b>");

...
export class SpaceHoldGesture {
	...
	#active = false;
	/** Latched hold: `held` while the keys still auto-repeat, then `released`, where a Space or
	 *  Backspace tap stops the recording. Undefined for an ordinary hold. */
	#latch: "held" | "released" | undefined;
	...
	/** Feed one keypress, `isSpace` when it is a plain space bar press and `isBackspace` when it is
	 *  Backspace. */
	process(isSpace: boolean, isBackspace = false): SpaceHoldStep {
		if (this.#latch !== undefined) {
			// Latched: every other key types normally and leaves the recording running.
			if (!isSpace && !isBackspace) return "pass";
			if (this.#latch === "held") this.#armReleaseTimer();
			else this.#end();
			return "swallow";
		}
		if (this.#active) {
	...
	#armReleaseTimer(): void {
		...
		this.#releaseTimer = setTimeout(() => {
			...
			if (this.#latch === "held") this.#latch = "released";
			else this.#end();

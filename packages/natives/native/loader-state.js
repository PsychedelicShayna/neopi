 ...
 *
 * Windows-only safety net for `bun install -g` updates: when a previous `npi`
 ...
 *   1. Gives every package version its own filesystem path, so concurrent NeoPi
...
// A concurrently starting older NeoPi binary creates or refreshes this directory
...
export function validateLoadedBindings(ctx, bindings, candidate) {
 ...
 if (residentSentinel && diskHasExpectedSentinel) {
  ...
  throw new Error(
   ...
   `\`${ctx.versionSentinelExport}\` this loader expects. npi was upgraded to ` +
   `${ctx.packageVersion} while this session was running; the ${residentVersion} addon is ` +
   "still resident in this process. Disk is already consistent — restart npi to pick up " +

 ...
 * - `dist/npi-browser-relay-extension.zip` — packaged extension for GH releases
 * - `../coding-agent/src/tools/browser/relay/extension-assets/*.txt` —
 *   generated text assets embedded into the npi CLI so `npi browser-relay
...
const zip = await $`zip -qr ../npi-browser-relay-extension.zip .`.cwd(distExtension).nothrow();
if (zip.exitCode !== 0) {
...
console.log(`  ${path.join(dist, "npi-browser-relay-extension.zip")}`);
